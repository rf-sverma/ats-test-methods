...
const jsdocTemplate = (methodName, description, parameters) => `
/**
 * @function ${methodName}
...
const tsdocTemplate = (methodName, description, parameters) => `
/**
 * ${description}
 * 
 * @param ${parameters}
...
const generateCategoryDocs = (category, methods, template) => {
    const categoryDocs = `# ${category} Methods\n\n`;
    let methodsDocs = "";
    for (const methodName in methods) {
        if (typeof methods[methodName] === "function") {
            const method = methods[methodName];
            const methodComments = method.toString().match(/\/\*\*(.|\n)*?\*\//);
